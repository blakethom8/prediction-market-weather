...
from weatherlab.ingest.nws_cli import ingest_nws_cli_text
from weatherlab.replay import replay_decision_for_market
...
class DecisionReplayTests(unittest.TestCase):
    def setUp(self):
        ...
        self.db_path = Path(self.tmpdir.name) / 'replay_test.duckdb'
    ...
    def test_replay_decision_for_market(self):
        ingest_contract(
        ...
        decision_id = replay_decision_for_market(market_ticker='TEST_NYC_70', db_path=self.db_path)

        con = connect(db_path=self.db_path)
        try:
            row = con.execute(
                'select action, abstain_reason, rationale_json from ops.decision_journal where decision_id = ?',
                [decision_id],
            ).fetchone()
        finally:
            con.close()

        self.assertEqual(row[0], 'BUY_YES')
        self.assertIsNone(row[1])
        self.assertIn('gate_summary', row[2])
